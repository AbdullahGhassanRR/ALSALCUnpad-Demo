"use client"
import { ArticleResponse, ArticleItem, ArticleWriting } from "./PostSameworkWriting";
import { useEffect, useState, useRef } from 'react';
import Link from 'next/link';
import Image from 'next/image';

export default function WritingByEdev() {
    ...
    useEffect(function () {
        ...
        async function loadList_WritingByEdev() {
            try {
                const response = await fetch('/api/publication-full/writing-by-edev', {
                    method: 'GET',
                    cache: 'no-store'
                });

                if (!response.ok) {
                    console.log("No response when fetching. inside async function loadList_WritingByEdev-> !response.ok");
                    return;
                }

                const payload = (await response.json()) as ArticleResponse;
                console.log("API Response:", payload); // Debugging log
                console.log("Raw Data from API:", payload.data); // Debugging log

                if (!payload.success || !payload.data || payload.data.length === 0) {
                    console.log("Failed in async function loadList_WritingByEdev-> !payload.success etc");
                    return;
                }
                ...
                const postSameworkList = payload.data
                    .filter(function (item) {
                        if (!item.slug || !item.slug.current) {
                            console.warn("Missing slug for item:", item); // Log missing slug
                            return false;
                        }
                        return Boolean(item.poster_image_url && item.title && item.created_at && item.slug.current);
                    })
                    .map(function (item) {
                        return {
                ...
                console.log("Filtered and Mapped Data:", postSameworkList); // Debugging log

                if(isMounted) setPostSameworkList(postSameworkList);

            } catch (error) {
                console.log("Failed in async function loadList_WritingByEdev", error);
            }
        }

        loadList_WritingByEdev();
    ...
    return (
        <div className="min-h-screen bg-[var(--primary-color)] py-10 px-4 sm:px-8">
            ...
            <div className="max-w-5xl mx-auto space-y-8">
                <h1 className="text-7xl font-bold text-[var(--secondary-color)] mb-17 text-center tracking-widest">
                    Writing by Edev
                </h1>
